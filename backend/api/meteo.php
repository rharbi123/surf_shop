...
try {
    ...
    // GET /spots/{nom}/meteo → météo actuelle + prochaines 24h
    if ($ressource === 'meteo') {

        // Prochaine entrée = météo "actuelle"
        $sqlActu = "SELECT m.id_meteo, m.date_heure, m.temperature, m.ressenti,
        ...
        $stmtActu->execute([$spot]);
        $actuelle = $stmtActu->fetch();

        // Prochaines 24h pour le graphique
        $sql24h = "SELECT m.id_meteo, m.date_heure, m.temperature, m.ressenti,
                          m.humidite, m.vent_vitesse, m.vent_direction, m.conditions, m.icone
                   FROM meteo m
                   JOIN spot s ON m.id_spot = s.id_spot
                   WHERE s.nom = ?
                   AND m.date_heure BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 24 HOUR)
                   ORDER BY m.date_heure ASC";

        $stmt24h = $pdo->prepare($sql24h);
        $stmt24h->execute([$spot]);
        $heures = $stmt24h->fetchAll();
